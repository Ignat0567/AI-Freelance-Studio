...
from order_workflow.production_adapter import LiveOpenCodeExecutionAdapter, ProductionProjectExecutionAdapter
...
def test_live_defaults_to_the_phased_adapter(tmp_path):
    adapter = ConfigurationBackedExecutionAdapter(_configuration(tmp_path), live=True)
...
def test_legacy_env_var_selects_the_legacy_adapter(tmp_path):
    adapter = ConfigurationBackedExecutionAdapter(_configuration(tmp_path), live=True, environ={"FREELANCERSTUDIO_EXECUTION_PIPELINE": "legacy"})
...
def test_unrecognized_pipeline_value_still_defaults_to_phased(tmp_path):
    adapter = ConfigurationBackedExecutionAdapter(_configuration(tmp_path), live=True, environ={"FREELANCERSTUDIO_EXECUTION_PIPELINE": "typo"})
...
def test_real_qa_backend_env_var_is_respected_even_though_live_opt_in_synthesizes_its_own_environ(tmp_path):
    ...
    adapter = ConfigurationBackedExecutionAdapter(_configuration(tmp_path), live=True, environ={"FREELANCERSTUDIO_PHASED_QA_BACKEND": "host"})
