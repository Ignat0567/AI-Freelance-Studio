...
What the loader cannot govern is the agent's own tool use. A `CLAUDE.md` left in the working directory is still a file the agent can open, and measurement shows it will: asked about "this project" in a directory holding one, it read the file and answered from it. A file whose entire purpose is to instruct an agent does not belong in a directory that will hold client-supplied templates. `scrub_agent_config` therefore removes both `.claude/` and `CLAUDE.md` from the project directory before every CLI invocation, not only the first, because a build phase can write either file and the repair calls that follow run in the same directory. Removals are reported into the run's event stream rather than performed silently. This is defence in depth; the flag above is what closes the loading path.

At delivery the same helper runs again, for every stack, removing `.claude/` alone. This is a distinct concern from everything above, and it does not depend on any of it: a `.claude/` in a shipped project would execute on the *client's* machine the moment they opened it in agent tooling. A `CLAUDE.md` the build wrote is documentation and ships with the rest.
