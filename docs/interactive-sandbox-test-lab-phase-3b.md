...
The static guest entry requests normal guest shutdown through canonical `shutdown.exe` only after it has atomically published `entry-payload-result.json` or controlled entry-failure evidence. The host treats the short-lived `WindowsSandbox.exe` return code as diagnostic only. Because preflight requires no active session, the host immediately pins the single canonical System32 `WindowsSandboxClient.exe` created as a direct child of the retained launcher, including exact PID, parent PID, image, and creation identity. After terminal validation and atomic host snapshot publication, it allows 15 seconds for guest shutdown and polls the active-session guard until the server and remote session disappear.

If guest shutdown does not complete after malformed evidence, nonzero payload result, timeout, cancellation, handled exception, or guest crash, the host revalidates the pinned client identity and first requests `CloseMainWindow`. After a five-second grace it may terminate only that exact creation-verified client and waits another 15 seconds for the active-session guard. It never uses global `taskkill`, process-name termination, or an unowned PID. A run cannot pass while the active-session guard still reports a Sandbox session. Launcher exit code and timing are recorded only as diagnostics after actual session termination.
