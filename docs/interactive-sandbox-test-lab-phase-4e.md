...
Originally, both profiles pointed at the same pinned self-installing artifact — there was no way to target an arbitrary customer-delivered installer through this bridge. `production_self_test` runs the Phase 3A install/launch/backend-health self-test; `production_screenshot` runs the Phase 3B screenshot self-test on top of the same install. Both are exercises of the harness against the app's own packaged output, not a generic "audit any installer" capability. A third profile, `interactive_session` (Phase 5a, see `docs/interactive-sandbox-test-lab-phase-5-design.md`), was added on top of this bridge later: it does not install or verify anything at all — it just opens a bare, unattended-timeout-capped Windows Sandbox window for direct human control.
