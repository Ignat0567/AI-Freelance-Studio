...
def _scan_meaningful_artifacts(workdir: str | None, *, max_files: int = 200, max_depth: int = 4, limit: int = 50) -> tuple[str, ...]:
    ...
    for child in sorted(root.rglob("*"), key=lambda item: str(item.relative_to(root)).casefold()):
        ...
        try:
            ...
            # those, and stat()-ing them raises OSError (WinError 1920) instead of just
            # reporting a type. Same issue and fix as order_workflow/workspace.py's
            # near-identical scan_meaningful_generated_artifacts().
            continue
        if is_dir:
