...
def test_missing_or_invalid_config_returns_existing_empty_key_behavior(monkeypatch, tmp_path):
    ...
    assert ai_utils.get_api_key("openai") == ""
    assert ai_utils.ask_studio_ai_with_history("opencode_bridge", "openai/gpt-5.5", "system", []) == "OpenCode bridge is not configured for this model. Add and test a local OpenCode connection in AI Providers."
