...
export default function OpenCodeConnectionSetup({ activePort, addLog, onSaved }) {
  ...
  const repair = async () => { const result = await request('/api/provider-connections/opencode/test'); if (result) { setTested(result); setMessage(result.ready ? 'Repair complete. OpenCode CLI, provider authentication, and selected model are ready.' : (result.message || 'Repair needs your action.')); } };
  const save = async () => {
  ...
  return <section className="provider-setup">
    ...
    <div className="provider-actions">{nodeMissing && <button type="button" onClick={() => openOfficialDownload('nodejs')} disabled={busy}>Download Node.js</button>}{opencodeMissing && <button type="button" onClick={() => openOfficialDownload('opencode')} disabled={busy}>Download OpenCode</button>}<button type="button" onClick={detect} disabled={busy}>Detect Again</button><button type="button" onClick={repair} disabled={busy || !draft.configured_model || !dependencies.opencode?.installed}>Repair automatically</button><button type="button" onClick={test} disabled={busy || !draft.configured_model || !dependencies.opencode?.installed}>Test Connection</button><button type="button" className="primary" onClick={save} disabled={busy || !tested?.ready}>Save Connection</button></div>
