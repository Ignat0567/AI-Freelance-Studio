      ...
      <section className="fs-app">
        ...
        <div className="fs-body">
          <main className={`fs-workspace ${activeView === 'overview' ? 'fs-overview-workspace' : ''} ${activeView === 'settings' ? 'fs-settings-workspace' : ''} ${activeView === 'info' ? 'fs-info-workspace' : ''}`} tabIndex={0} aria-label="Central workspace content">
            {!['settings', 'info', 'sandbox', 'create-project'].includes(activeView) && <ProjectOverview project={project} primaryAction={primaryAction} isGenerating={isGenerating} onStopGeneration={onStopGeneration} onNewProject={onNewProject} />}
            {activeView === 'overview' && (
              <>
            ...
            {activeView !== 'overview' && !['create-project', 'team', 'projects', 'features', 'pipeline', 'mobile', 'issues', 'logs', 'sandbox', 'settings', 'info'].includes(activeView) && <WorkspaceHint activeView={activeView} project={project} />}
