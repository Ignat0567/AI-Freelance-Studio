...
def test_unapproved_salon_inventory_is_not_public_or_bookable():
    with main.db() as conn:
        ts = main.now_utc()
        owner_id = conn.execute("INSERT INTO users(email,password_hash,first_name,last_name,role,created_at,updated_at) VALUES(?,?,?,?,?,?,?)", ("hidden-owner@beautylip.dev", main.hash_password("BeautyLip123!"), "Hidden", "Owner", "manager", ts, ts)).lastrowid
        pro_id = conn.execute("INSERT INTO users(email,password_hash,first_name,last_name,role,created_at,updated_at) VALUES(?,?,?,?,?,?,?)", ("hidden-pro@beautylip.dev", main.hash_password("BeautyLip123!"), "Hidden", "Pro", "professional", ts, ts)).lastrowid
        salon_id = conn.execute("INSERT INTO salons(owner_id,name,city,approval_status,is_active,created_at,updated_at) VALUES(?,?,?,?,?,?,?)", (owner_id, "Hidden Salon", "Berlin", "pending", 1, ts, ts)).lastrowid
        branch_id = conn.execute("INSERT INTO salon_branches(salon_id,name,city,is_active) VALUES(?,?,?,1)", (salon_id, "Hidden Branch", "Berlin")).lastrowid
        specialist_id = conn.execute("INSERT INTO specialists(user_id,salon_branch_id,display_name,is_active) VALUES(?,?,?,1)", (pro_id, branch_id, "Hidden Pro")).lastrowid
        service_id = conn.execute("INSERT INTO services(salon_branch_id,name,duration_minutes,price,is_active) VALUES(?,?,?,?,1)", (branch_id, "Hidden Service", 30, 10)).lastrowid
        conn.execute("INSERT INTO specialist_services(specialist_id,service_id,is_active) VALUES(?,?,1)", (specialist_id, service_id))
    assert all(item["id"] != service_id for item in client.get("/api/services").json())
    assert client.get(f"/api/specialists/{specialist_id}").status_code == 404
    customer = auth("customer@beautylip.dev")
    assert client.post(f"/api/favorites/specialists/{specialist_id}", headers=customer).status_code == 404
    today = date.today().isoformat()
    assert client.get(f"/api/availability?specialist_id={specialist_id}&service_id={service_id}&date={today}").status_code == 404
    payload = {"salon_branch_id": branch_id, "specialist_id": specialist_id, "service_id": service_id, "start_at": f"{today}T10:00:00+00:00"}
    assert client.post("/api/appointments", json=payload, headers=customer).status_code == 404
