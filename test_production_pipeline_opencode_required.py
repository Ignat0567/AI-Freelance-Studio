...
def test_alternative_coding_backend_restricted_to_real_file_editing_connections():
    assert "_CODING_CAPABLE_CONNECTION_TYPES = {" in MAIN_SOURCE
    assert "ConnectionType.CODEX_CHATGPT_SUBSCRIPTION.value" in MAIN_SOURCE
    assert "ConnectionType.CLAUDE_SUBSCRIPTION.value" in MAIN_SOURCE
    assert "ConnectionType.OPENCODE_PROVIDER.value" in MAIN_SOURCE
    ...
    assert "OPENAI_API_KEY" not in MAIN_SOURCE.split("_CODING_CAPABLE_CONNECTION_TYPES = {")[1].split("}")[0]
