"""Isolated AI API caller — runs in separate process to avoid SSL crashes in uvicorn.
Uses PowerShell for HTTPS since Python OpenSSL on Windows Store Python 3.13 is broken.
Reads payload from stdin to avoid PowerShell quote-stripping.
Includes retry logic for slow NVIDIA API."""
import sys
import os
import json
import subprocess
...
def main():
    if len(sys.argv) != 3:
        print("Usage: _ai_worker.py <url> <api_key>  (reads payload_json from stdin)", file=sys.stderr)
        sys.exit(1)
...
try {{
    ...
    $client.Timeout = New-Object System.TimeSpan(0, 0, 90)
    $client.DefaultRequestHeaders.Add("Authorization", "Bearer {safe_api_key}")
