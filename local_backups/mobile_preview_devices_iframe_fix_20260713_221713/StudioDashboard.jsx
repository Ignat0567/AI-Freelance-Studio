...
const MOBILE_DEVICES = [
  { id: 'iphone15', name: 'iPhone 15 Pro', width: 393, height: 852, radius: 46 },
  { id: 'iphoneSE', name: 'iPhone SE', width: 375, height: 667, radius: 28 },
  { id: 'samsungS24', name: 'Samsung Galaxy S24', width: 360, height: 780, radius: 34 },
  { id: 'pixel8', name: 'Google Pixel 8', width: 412, height: 915, radius: 34 },
  { id: 'ipad', name: 'iPad Mini', width: 744, height: 1133, radius: 28 },
];

function MobilePreviewPanel({ project }) {
  const [deviceId, setDeviceId] = useState('iphone15');
  const [previewUrl, setPreviewUrl] = useState(project?.preview_url || project?.url || '');
  const device = MOBILE_DEVICES.find(item => item.id === deviceId) || MOBILE_DEVICES[0];
  const scale = device.width > 430 ? 0.58 : 0.76;
  return (
    <section className="fs-panel fs-mobile-preview-page">
      ...
      <div className="fs-mobile-preview-layout">
        <div className="fs-mobile-controls">
          ...
          <p>Use this for responsive web, Expo web, React Native Web, Ionic, Capacitor, or any generated mobile project that exposes a local preview URL. Native Android/iOS emulator launch depends on Android SDK or Xcode/iOS Simulator in Info diagnostics.</p>
        </div>
        <div className="fs-phone-stage">
          <div className="fs-phone-frame" style={{ width: device.width * scale, height: device.height * scale, borderRadius: device.radius * scale }}>
            ...
            {previewUrl ? <iframe title={`${device.name} preview`} src={previewUrl} style={{ borderRadius: Math.max(18, device.radius * scale - 10) }} /> : <div className="fs-phone-empty"><b>{device.name}</b><span>Enter a running local project URL to preview it in this virtual phone.</span></div>}
          </div>
        </div>
      </div>
    </section>
