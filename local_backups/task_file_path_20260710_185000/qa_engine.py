...
def _walk_project_files(root_path):
    for root, dirs, files in os.walk(root_path):
        ...
        for fname in files:
            if os.path.splitext(fname)[1].lower() in IGNORED_QA_EXTENSIONS:
                continue
            ...
            try:
                ...
                if "build" in scripts:
                    if not os.path.isdir(os.path.join(self.target_path, "node_modules")):
                        npm_install = self.run_command(["npm", "install"], cwd=self.target_path, timeout=180)
                    ...
                    if not any(e.startswith("npm install failed") for e in errors):
                        npm_build = self.run_command(["npm", "run", "build"], cwd=self.target_path, timeout=180)
...
'''
    ...
    def _requirement_gap_blocker(self):
        ...
        for category, state in (("missing_credential", "needs_credentials"), ("ambiguity", "needs_human_input")):
            blockers = [
                gap for gap in gaps
    ...
    def _request_opencode_fix(self, errors, error_text, repair_report=None):
        ...
        try:
            ...
            if changed:
                ...
                result.update(status="subprocess_timeout" if timed_out else ("subprocess_nonzero_exit" if not success else "subprocess_zero_exit_no_changes"), failure_stage="subprocess", error_category="timeout" if timed_out else ("nonzero_exit" if not success else "no_source_changes"))
                return finish()
        except Exception as e:
