...
def test_targeted_acceptance_verifier_registry_covers_hardening_criteria():
    expected = {
        "AC-003", "AC-004", "AC-006", "AC-007", "AC-010", "AC-012", "AC-013",
        "AC-014", "AC-015", "AC-018", "AC-019", "AC-020", "AC-021",
    }

    assert expected <= set(delivery_audit.TARGETED_ACCEPTANCE_VERIFIERS)
...
def test_targeted_acceptance_verifier_is_gated_to_real_target_path(tmp_path):
    criterion = {"id": "AC-004", "title": "The application is usable through a browser."}
    project = {"acceptance_criteria": [criterion], "project_profiles": ["fastapi"], "project_spec": {"project_profiles": ["fastapi"]}}

    assert delivery_audit._targeted_acceptance_enabled(criterion, project, str(tmp_path)) is False
    project["target_path"] = str(tmp_path)
    assert delivery_audit._targeted_acceptance_enabled(criterion, project, str(tmp_path)) is True
