...
function GlobalAIInheritanceSettings({ activePort, cfg, reload, addLog }) {
  ...
  const [message, setMessage] = useState('');
  const [busy, setBusy] = useState(false);
  const connections = cfg.provider_connections || [];
  const selected = connections.find(c => c.connection_id === connectionId) || connections[0];
  const models = selected?.available_models || [];

  const loadAgents = () => fetch(`http://localhost:${activePort}/api/agents`).then(r => r.json()).then(setAgents).catch(() => setAgents({}));
  useEffect(() => { setGlobalAI(cfg.global_ai || {}); setConnectionId(cfg.global_ai?.connection_id || connections[0]?.connection_id || ''); setModel(cfg.global_ai?.model || connections[0]?.available_models?.[0]?.id || ''); setTemperature(cfg.global_ai?.temperature ?? 0.2); setTopP(cfg.global_ai?.top_p ?? ''); setTopK(cfg.global_ai?.top_k ?? ''); setMaxTokens(cfg.global_ai?.max_tokens ?? ''); loadAgents(); }, [activePort, cfg]);
  ...
  const saveGlobal = () => {
    ...
    fetch(`http://localhost:${activePort}/api/config/ai/global`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ connection_id: connectionId, provider: conn?.provider || globalAI.provider || cfg.provider, model, temperature: parseFloat(temperature), top_p: topP === '' ? null : parseFloat(topP), top_k: topK === '' ? null : parseInt(topK, 10), max_tokens: maxTokens === '' ? null : parseInt(maxTokens, 10), enabled: true }) })
      .then(r => r.json()).then(data => { setMessage('Global AI configuration saved.'); addLog?.(`[AI Settings]: Global model set to ${data.global_ai?.provider}/${data.global_ai?.model}.`); reload(); loadAgents(); })
  ...
  const applyAll = () => {
    ...
    fetch(`http://localhost:${activePort}/api/agents/apply-global-inheritance`, { method: 'POST' })
      .then(r => r.json()).then(data => { setMessage(`Applied to ${data.updated?.length || 0} agent(s). Skipped ${data.skipped?.length || 0}.`); addLog?.('[AI Settings]: Applied global inheritance to agents.'); loadAgents(); })
  ...
  const updateAgent = (agentId, patch) => {
    ...
    fetch(`http://localhost:${activePort}/api/agents/${agentId}/config`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(patch) })
      .then(r => r.json()).then(() => loadAgents()).catch(err => setMessage(`Agent save failed: ${err.message}`)).finally(() => setBusy(false));
  };

  return <section className="provider-setup"><div className="provider-heading"><div><strong>Global AI Configuration</strong><p>One Studio provider/model can power all compatible agents. Temperature and sampling can still be overridden per agent.</p></div><span className="connection-state">{globalAI.provider || cfg.provider}/{globalAI.model || cfg.model}</span></div><div className="provider-form"><label>Provider connection<select value={connectionId} disabled={busy} onChange={e => { const next = connections.find(c => c.connection_id === e.target.value); setConnectionId(e.target.value); setModel(next?.available_models?.[0]?.id || ''); }}><option value="">Select provider connection</option>{connections.map(c => <option key={c.connection_id} value={c.connection_id}>{c.display_name || c.name || c.provider}</option>)}</select></label><label>Default model<select value={model} disabled={busy} onChange={e => setModel(e.target.value)}><option value="">Select model</option>{models.map(m => <option key={m.id} value={m.id}>{m.id}</option>)}</select></label><label>Default temperature<input type="number" min="0" max="2" step="0.05" value={temperature} onChange={e => setTemperature(e.target.value)} /></label><label>Default top_p<input type="number" min="0" max="1" step="0.05" value={topP ?? ''} onChange={e => setTopP(e.target.value)} /></label><label>Default top_k<input type="number" min="1" step="1" value={topK ?? ''} onChange={e => setTopK(e.target.value)} /></label><label>Default max tokens<input type="number" min="1" step="1" value={maxTokens ?? ''} onChange={e => setMaxTokens(e.target.value)} /></label></div><div className="provider-actions"><button type="button" className="primary" onClick={saveGlobal} disabled={busy || !connectionId || !model}>Save global configuration</button><button type="button" onClick={applyAll} disabled={busy}>Apply inheritance to all agents</button></div>{message && <p className="provider-message" role="status">{message}</p>}<div className="mt-3 space-y-2">{Object.entries(agents).map(([agentId, agent]) => { const eff = agent.effective_ai || {}; return <div key={agentId} className="judge-details"><b>{agent.name || agentId}</b><span>Model: <b>{eff.model || '-'}{eff.inherited_fields?.model ? ' - inherited from Global' : ''}</b></span><span>Temp: <b>{eff.temperature ?? '-'}</b></span><span>Capability: <b>{eff.capability_status || '-'}</b></span><label><input type="checkbox" checked={!!eff.use_global_connection} onChange={e => updateAgent(agentId, { use_global_connection: e.target.checked, use_global: e.target.checked && eff.use_global_model })} /> Global connection</label><label><input type="checkbox" checked={!!eff.use_global_model} onChange={e => updateAgent(agentId, { use_global_model: e.target.checked, use_global: e.target.checked && eff.use_global_connection })} /> Global model</label><label><input type="checkbox" checked={!!eff.use_global_generation_parameters} onChange={e => updateAgent(agentId, { use_global_generation_parameters: e.target.checked })} /> Global parameters</label><input type="number" min="0" max="2" step="0.05" value={eff.temperature ?? ''} disabled={eff.use_global_generation_parameters} onChange={e => updateAgent(agentId, { temperature: parseFloat(e.target.value), use_global_generation_parameters: false })} title={eff.use_global_generation_parameters ? 'Inherited from Global' : 'Agent temperature override'} /><button type="button" onClick={() => updateAgent(agentId, { use_global_connection: true, use_global_model: true, use_global_generation_parameters: false, use_global: true })}>Reset to defaults</button></div>; })}</div></section>;
}
