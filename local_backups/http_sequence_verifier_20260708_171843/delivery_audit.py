...
from project_spec import acceptance_evidence_is_direct, detect_project_profiles, ensure_acceptance_evidence_history, normalize_acceptance_evidence, record_acceptance_evidence
...
def _evaluate_acceptance(project: dict, root: str, qa_result: dict | None, checks: list[dict[str, Any]]) -> tuple[bool, list[dict[str, Any]]]:
    ...
    for criterion in project.get("acceptance_criteria", []):
        method = str(criterion.get("verification_method", "") or "").strip()
        unsupported_verifier = method not in ACCEPTANCE_VERIFIERS
