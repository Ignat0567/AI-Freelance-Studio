...
def _start_ticket_runtime(criterion: dict[str, Any], project: dict, root: str) -> tuple[RuntimeServerHandle | None, dict[str, Any], dict[str, Any], dict[str, str]]:
    ...
    login_routes = [path for path, item in openapi_response["json"].get("paths", {}).items() if "login" in str(path).lower() and isinstance(item, dict) and "post" in item]
    if login_routes:
        # A local-only verifier may use documented development credentials without recording them.
        login = _http_json_request(handle.base_url, "POST", login_routes[0], {"username": os.getenv("TICKET_TRACKER_ADMIN_USER", "admin"), "password": os.getenv("TICKET_TRACKER_ADMIN_PASSWORD", "admin123")})
        cookies = {}
        for header in login.get("set_cookie", []):
            name_value = header.split(";", 1)[0]
            if "=" in name_value:
                name, value = name_value.split("=", 1)
                cookies[name] = value
        handle.cookies = cookies
        collected["auth_plan"] = {"auth_required": True, "auth_type": "cookie_session", "login_route": login_routes[0], "authenticated": bool(login.get("ok") and cookies), "credential_source": "local_verifier_environment"}
...
def _build_create_payload(openapi: dict[str, Any], create_route: dict[str, Any], marker: str) -> tuple[dict[str, Any], str, str]:
    ...
    for field_name in ("status", "priority"):
        ...
        if not values:
            return {}, "", f"enum_discovery_failed:{field_name}"
