...
class LiveOpenCodeExecutionAdapter(ProductionProjectExecutionAdapter):
    ...
    def execute(self, request: ExecutionRequest, event_sink: ExecutionEventSink, cancellation: CancellationToken) -> ExecutionResult:
        ...
        delivery_summary = (
            "OpenCode created project files but timed out; manual review required and QA was not run."
            if result.outcome == "generated_needs_review"
            else "OpenCode timed out without generated project files; QA was not run."
            if result.outcome == "timed_out_without_artifacts"
            else "Live OpenCode execution completed; QA was not run."
            if result.success
        ...
        return ExecutionResult(
            ...
            outcome=result.outcome if result.success else "timed_out_without_artifacts" if result.outcome == "timed_out_without_artifacts" else "failed",
            summary="OpenCode created project files but did not exit before timeout. Review the workspace before QA." if result.outcome == "generated_needs_review" else "Project generated by live OpenCode execution. QA was not run in this live opt-in commit." if result.success else "OpenCode did not finish and no generated project files were detected." if result.outcome == "timed_out_without_artifacts" else "Live OpenCode execution did not complete successfully.",
            artifact_ids=tuple(item.id for item in artifacts),
            test_summary=TestSummary(skipped=1),
            warnings=("QA was not run in this live opt-in commit.", *result.warnings),
            errors=() if result.success else result.errors or ("opencode_execution_failed",),
