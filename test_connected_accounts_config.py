...
def test_connected_accounts_are_read_and_written_via_config_storage(monkeypatch, tmp_path):
    ...
    assert set(stored["_accounts"][0]) == set(connected_accounts.ACCOUNT_METADATA_FIELDS)


def test_legacy_credentials_are_not_returned_and_have_safe_warning(monkeypatch, tmp_path):
    config_path = tmp_path / "studio_config.json"
    monkeypatch.setattr(config_storage, "CONFIG_FILE", str(config_path))
    ...
    accounts = connected_accounts.get_accounts()

    assert accounts[0]["warning"] == "legacy_credentials_present"
    assert "credentials" not in accounts[0]
    assert "redacted" not in str(accounts)
