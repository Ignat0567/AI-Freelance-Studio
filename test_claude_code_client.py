...
def test_removing_workspace_agent_config_is_reported_not_silent(monkeypatch, tmp_path):
    """Agent configuration in a generated project is either something the build did that
    nobody asked for, or something that arrived with the order. The second is worth a human
    seeing, so it goes in the event stream rather than being quietly tidied away."""
    ...
    ConfiguredClaudeCodeExecutionClient().execute_project_prompt("build it", tmp_path, sink, CancellationToken())

    assert any("CLAUDE.md" in message for message in sink.messages)


def test_a_clean_workspace_reports_nothing(monkeypatch, tmp_path):
    """The warning has to mean something when it appears, which it stops doing if every
    ordinary build emits it."""
    fake_popen = _FakePopen({"is_error": False, "result": "done"})
    monkeypatch.setattr("order_workflow.claude_code_client.subprocess.Popen", fake_popen)
    monkeypatch.setattr("order_workflow.claude_code_client._ensure_isolated_git_repo", lambda _path: None)
    sink = _RecordingSink()
    ...
    assert not any("Removed agent configuration" in message for message in sink.messages)
