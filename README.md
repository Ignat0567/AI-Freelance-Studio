...
3. On execution: pick **Who writes the project files** (Grok subscription is the live path used in Day 3). Tick **Continue?** The checklist above the start button must be ok, not blocked. **Refresh readiness** if you just logged in or started Docker.
...
|---|---|
| Grok CLI | `grok login` in a terminal (the same `grok` as PowerShell) |
